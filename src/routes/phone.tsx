...
export const Route = createFileRoute("/phone")({
  head: () => ({
    meta: [
      { title: "Enter Mobile Number — Nectar" },
    ],
  }),
  component: Phone,
});
...
function Phone() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F3F2] lg:py-12 lg:px-4 flex flex-col justify-center">
      <div className="animate-page-in min-h-screen bg-white flex flex-col relative lg:max-w-md lg:mx-auto lg:w-full lg:h-[90vh] lg:max-h-[800px] lg:min-h-[650px] lg:py-8 lg:rounded-[30px] lg:overflow-hidden lg:shadow-2xl">
      ...
      <div className="px-6 pt-12 flex-1 relative z-10">
        ...
        <div className="mt-2 flex items-center gap-3 border-b border-[#E2E2E2] py-3">
          <img src="https://flagcdn.com/w40/in.png" alt="IN Flag" className="h-[20px] w-[28px] rounded-sm object-cover" />
...
export function IOSKeypad({ onDigit, onBack }: { onDigit: (d: string) => void; onBack: () => void }) {
  return (
    <div className="bg-[#D1D4D9] pb-8 pt-2 px-1.5 backdrop-blur-xl bg-opacity-90">
      <div className="grid grid-cols-3 gap-1.5">
        ...
        <button 
          className="flex items-center justify-center text-xl font-medium text-black transition active:bg-black/10 rounded-lg"
        >
          + * #
        </button>
...
function KeypadButton({ main, sub, onClick }: { main: string, sub: string, onClick: () => void }) {
  return (
    ...
    >
      ...
      {sub && <span className="text-[10px] leading-none font-bold tracking-widest text-black mt-0.5">{sub}</span>}
    </button>
  );
}
